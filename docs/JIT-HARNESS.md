...
This package expresses **GitHub Issues** as a typed, task-adaptive agent harness. Its concrete capability remains: Create, triage, label, and assign GitHub issues through guarded gh or REST workflows. The harness contract makes memory, planning, action, capability selection, failure handling, and verification explicit.
...
| Module | Strategy | Responsibility in GitHub Issues |
| --- | --- | --- |
| Memory | `stage_artifact_log` | Retain issue request plus decisions, observations, and verification evidence needed to resume safely. |
| Planning | `linear_gated_plan` | Transform repository and labels into the ordered, bounded stages defined by this skill. |
...
| Action | `inspect_preview_authorize_execute_reread` | Advance the workflow through issue draft while preserving stop conditions and user authority. |
